...
/// driver. A caller asking more than once should hold onto the answer.
///
...
mod imp {
    ...
    pub(in super::super) fn probe() -> bool {
        ...
        // `library`.
        unsafe { devices(&library) }.is_some_and(supports_webgpu)
    }

    /// Creates an instance, reads every physical device's version and type, and destroys it again.
    ///
    /// # Safety
    ///
    /// `library` must be the Vulkan loader.
    unsafe fn devices(library: &Library) -> Option<Vec<(u32, i32)>> {
