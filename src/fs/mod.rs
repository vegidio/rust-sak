...
//! hostile. Entry names are validated identically on every platform, and each
...
//! a panic, so nothing leaks when a function returns down an unexpected path. Both are this crate's own types
//! wrapping `tempfile`, so that crate's major version stays out of this one's public API.
//!
//! ```
